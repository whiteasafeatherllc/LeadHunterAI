...
from scrapers.reddit import search_posts as reddit_search
from scrapers.twitter_like import search_twitter, search_instagram, search_tiktok

app = FastAPI(title="LeadHunterAI — Social Lead Finder (MVP)", version="0.1.0")
...
def search(keyword: str, platforms: Optional[str] = "twitter,reddit,instagram,tiktok", max_results: int = 15):
    ...
    if "reddit" in platforms_list:
        try:
            for r in reddit_search(keyword, max_results=max_results):
                r["platform"] = "reddit"
    ...
    if "twitter" in platforms_list:
        try:
            for r in search_twitter(keyword, max_results=max_results):
                r["platform"] = "twitter"
                all_results.append(r)
        except Exception:
            pass
    if "instagram" in platforms_list:
        try:
            for r in search_instagram(keyword, max_results=max_results):
    ...
    if "tiktok" in platforms_list:
        try:
            for r in search_tiktok(keyword, max_results=max_results):
                r["platform"] = "tiktok"
                all_results.append(r)
        except Exception:
            pass
